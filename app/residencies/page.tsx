...
import { ArrowRight, Sparkles, TrendingUp, Compass, Building2, Home } from 'lucide-react'
import { Header } from '@/components/header'
import { Footer } from '@/components/footer'
import { allResidencies } from '@/data/residencies'
import {
  RESIDENCY_CATEGORY_META,
  getResidencyFocusByPopularity,
  getResidencyFocusStats,
  getResidenciesForFocus,
...
export default function ResidenciesPage() {
  ...
  const focusAreas = useMemo(() => getResidencyFocusByPopularity(), [])
  const stats = useMemo(() => getResidencyFocusStats(), [])

  const focusedResidencies = useMemo(() => {
    if (!selectedFocus) return allResidencies
    return getResidenciesForFocus(selectedFocus)
  }, [selectedFocus])

  const visibleResidencies = useMemo(() => {
    if (!showOpenOnly) return focusedResidencies
    return focusedResidencies.filter(
      (r) => r.applicationStatus === 'open' || r.applicationStatus === 'rolling'
    )
  }, [focusedResidencies, showOpenOnly])

  const categorizedResidencies = useMemo(() => {
    const categories = new Map<string, typeof allResidencies>()
    for (const residency of visibleResidencies) {
      const existing = categories.get(residency.category) || []
      existing.push(residency)
      categories.set(residency.category, existing)
    }
    return categories
  }, [visibleResidencies])
  ...
  const categoryItems: CategoryItem[] = useMemo(
    () =>
      focusAreas.map((focus) => ({
        slug: focus.slug,
        name: focus.name,
        count: focus.residencyCount,
        totalCount: focus.totalResidencies,
        consensusPercentage: focus.consensusPercentage,
        color: focus.color,
        entities: focus.residencies.map((r) => ({
          slug: r.slug,
          shortName: r.shortName,
          brandColor: r.brandColor,
          name: r.name,
        })),
        detailUrl: `/residencies/focus/${focus.slug}`,
      })),
    [focusAreas]
  )
  ...
  const quickLinks: QuickLinkEntity[] = useMemo(
    () =>
      visibleResidencies.map((residency) => ({
        slug: residency.slug,
    ...
    <div className="min-h-screen pt-16">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            '@type': 'CollectionPage',
            name: 'Founder Residencies 2026',
            description:
              'A curated guide to top founder residency programs from leading VCs and communities.',
            url: 'https://openquest.com/residencies',
            mainEntity: {
              '@type': 'ItemList',
              numberOfItems: allResidencies.length,
              itemListElement: allResidencies.map((r, i) => ({
                '@type': 'ListItem',
                position: i + 1,
                name: r.name,
      ...
      <div className="relative overflow-hidden border-b">
        ...
        <div className="container relative mx-auto max-w-6xl px-4 py-16 sm:py-24">
          <nav className="mb-8 flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>/</span>
            <span className="text-foreground font-medium">Residencies</span>
          </nav>
          ...
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              ...
              >
                Compare top VC-backed residency programs for the -1 to 0 stage. Filter by
                focus area, check application status, and find the right environment to build conviction.
              </motion.p>
            </div>
          </div>
      ...
      <div id="focus-areas" className="container mx-auto max-w-6xl px-4 py-12">
        <div className="text-center mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold mb-2">Residency Focus Areas</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Click a focus area to filter programs. Bubble size reflects concentration among
            residency programs.
          </p>
        </div>
        ...
        <CategoryInfographic
          ...
          labels={{
            entityNoun: 'residencies',
            emergingLabel: 'Specialized focus areas',
            viewAllLabel: 'View all {name} residencies',
            selectedPrefix: 'Showing residencies for:',
          }}
        />
      </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold">
              {selectedFocus
                ? `Residencies in ${focusAreas.find((a) => a.slug === selectedFocus)?.name}`
                : 'All Residencies'}
            </h2>
            <p className="text-muted-foreground">
              {visibleResidencies.length} programs in current view
            </p>
          </div>
      ...
      <div className="container mx-auto max-w-6xl px-4 py-12 border-t">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Programs organized by model — VC-backed residencies, community cohorts, and more.
          </p>
        </div>

        <div className="space-y-12">
          {Array.from(categorizedResidencies.entries()).map(([category, residencies]) => {
            const meta = RESIDENCY_CATEGORY_META[category as keyof typeof RESIDENCY_CATEGORY_META]
            if (!meta) return null
            return (
              <div key={category}>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-sm text-muted-foreground">({residencies.length})</span>
                </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {residency.applicationStatus && residency.applicationStatus !== 'unknown' && (
                              <span className={getStatusBadgeClass(residency.applicationStatus)}>
                                {residency.applicationStatus === 'rolling'
                                  ? 'Rolling'
                                  : residency.applicationStatus === 'open'
                                    ? 'Open'
                                    : 'Closed'}
                              </span>
                            )}
                          ...
                          {residency.lastVerifiedAt && (
                            <p className="text-[11px] text-muted-foreground/80 mt-2">
                              Verified {new Date(residency.lastVerifiedAt).toLocaleDateString('en-US')}
                            </p>
                          )}
                          <p className="text-[11px] text-muted-foreground/80 mt-1">
                            {residency.duration} · {residency.location}
                          </p>
                        </div>
                      </Link>
                    </motion.div>
      ...
      <div className="border-t bg-muted/20 py-16">
        <div className="container mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-3xl font-bold mb-4">Find problems worth building</h2>
          <p className="text-lg text-muted-foreground mb-8">
            Use this guide to pick a residency, then validate your idea against
